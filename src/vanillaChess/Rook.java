...
public class Rook extends Piece {
	...
	public Integer[] exception(LinBoard board) {
		return ArrayMan.concatAll(move(board, false, true), move(board, true, true), move(board, false, false),
	...
	private Integer[] move(LinBoard board, boolean right, boolean rank) {
		...
		for (int i = this.getLoc() + board.vecToInteger(direction); comp(i, endpoint,
				!rank); i += board.vecToInteger(direction)) {
